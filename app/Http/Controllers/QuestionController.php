...
class QuestionController extends Controller
{
    ...
    public function genggai(Request $request, $id){
        ...
        $topics = DB::table('topics')
            ->where('id', $id)
            ->update(['name' => $request->name,'topic_pic'=>$file_name,'desc'=>$request->desc]);

        $file->move($destinationPath, $file_name);
        $topic_pic = Topic::where('id',$id)->first();
        $topic_pic->topic_pic = '/' . $destinationPath . $file_name;
        $topic_pic->save();
        

        if($topics){
            flash('更改话题成功','success');
        }else{
            flash('更改话题失败','error');
        }

        return redirect()->action('QuestionController@topic',['id'=>$request->id]);
    }
    ...
    public function topic($id){
        ...
        $comment = Comment::where('commentable_id',$id)->get();
        return view('questions.topic',compact('topics','topic','comment'));
